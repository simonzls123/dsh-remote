---
name: remote-dsh
description: 把任务投递到 192.168.3.19（WIN-0HCOS5TO4J1）上常驻的 dsh web，在远端创建原生 DSH 会话执行并取回结果；会话在远端 GUI 实时可见、可续聊、可接管。
whenToUse: 当用户出现「远程」「远端」「另一台电脑」「B 机」「19」等把任务交给远程机器执行的表述，或明确要求在 192.168.3.19 / WIN-0HCOS5TO4J1 上工作时。
---

# remote-dsh：用 HTTP RPC 驱动另一台电脑上的 dsh

远端 `192.168.3.19:3080` 运行着常驻 dsh web，它的 `/api` RPC 通道对 LAN 开放（trusted-hosts 自动放行），**无需任何脚本，直接用 pwsh 调 HTTP 即可**在远端创建原生会话。

## 协议信封（所有调用统一）

```http
POST http://192.168.3.19:3080/api/<method>
...
- 产物默认写远端 `C:\Users\administrator\Desktop\`；从远端取回：`net use \\192.168.3.19\C$ 'Password01!' /user:'insight\administrator'` 后 Copy-Item
